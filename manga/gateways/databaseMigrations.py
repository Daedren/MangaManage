...
class DatabaseMigrations:
    ...
    def __executeMigration(self, conn, currentVersion):
        if currentVersion == 0:
            ...
            self.logger.error("This version of the application is too old to run this database")
            return

    def __version0To1(self, conn: sqlite3.Connection):
        self.logger.debug("Executing migration version 0 -> 1")
        query = '''
            CREATE TABLE anilist(id integer primary key, series text unique, anilistId int);

            CREATE TABLE manga(id integer primary key, archive text unique, source text unique, chapter text, series text, creation_date datetime default CURRENT_TIMESTAMP);
